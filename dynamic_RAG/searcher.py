...
class Searcher:
            ...
            for index in range(len(r_squence)-1):
                ...
                for pre_rule in candidate_pre_rule:
                    ...
                    for aft_rule in candidate_aft_rule:
                        same = 0
                        if aft_rule[0] == pre_rule[0]:  # subject-type matches
                            same += 1
                        if aft_rule[1] == pre_rule[1]:  # relation matches
                            same += 1
                        if aft_rule[2] == pre_rule[2]:  # object-type matches
                            same += 1
                        if same < 2:
                            continue
    ...
    def build_temporal_model(self, raw_model, passes=1, verbosity=1000000):
        ...
        # >>> 新增：构建 rule<->fact 双向映射 <<<
        tmp_temporal_model.build_fact_rule_mappings()
