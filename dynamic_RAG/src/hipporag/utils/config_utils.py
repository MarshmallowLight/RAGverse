...
class BaseConfig:
    ...
    dynamic: bool = field(
        default=False,
        metadata={"help": "If set to True, will trigger the dynamic algorithm for temporal model."}
    )
    # Storage specific attributes 
    ...
    linking_top_k: int = field(
        default=10,
        metadata={"help": "The number of detected node at the start, prefer large than 20 to ensure the accuracy"}
    )
    retrieval_top_k: int = field(
        default=10,
        metadata={"help": "Retrieving k documents at each step, prefer a smaller one to save the token and make LLM more easy to understand"}
    )
    damping: float = field(
        default=0.5,
    ...
    temporal_dir: str = field(
        default=None,
        metadata={"help": "Directory to load the temporal rule model."}
    )
    ...
    # )
    dataset: Optional[Literal['multitq']] = field(
        default='multitq',
        metadata={"help": "Dataset to use. If specified, it means we will run specific datasets. If not specified, it means we're running freely."}
    )
