...
def _any_hit_at_k(pred_list: List[str], gold_list: List[str], k: int) -> float:
    """命中则返回 1.0，否则 0.0；pred_list 已经是候选有序列表"""
    gold_norm = set(normalize_answer(g) for g in gold_list)
    for cand in pred_list[:k]:
        if normalize_answer(cand) in gold_norm:
            return 1.0
    return 0.0

def evaluate_hit_at_k_f1(
    gold_answers: List[List[str]],
    predicted_topk_answers: List[List[str]],
    k: int
) -> Tuple[Dict[str, float], List[Dict[str, float]]]:
    """返回 pooled/逐样本的 Hit@k-F1（即 0/1 的平均，命中为 1）"""
    example = []
    total = 0.0
    for gold, preds in zip(gold_answers, predicted_topk_answers):
        score = _any_hit_at_k(preds, gold, k)
        example.append({"F1": score})
        total += score
    pooled = {"F1": total / len(gold_answers) if gold_answers else 0.0}
    return pooled, example