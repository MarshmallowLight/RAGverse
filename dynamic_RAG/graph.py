...
class Graph:
    ...
    def __init__(self, name, ext='txt', delimiter='	', idify=False, verbose=True, load_candidates_from_disk=False):
        ...
        edgelist = open(os.path.join(ROOT_DIR, 'reproduce/dataset/{}/train.{}'.format(name, ext)), 'r')
        typelist = open(os.path.join(ROOT_DIR, 'reproduce/dataset/{}/types_bad.{}'.format(name, ext)), 'r')
        label_map = open(os.path.join(ROOT_DIR, 'reproduce/dataset/{}/e2types_bad.{}'.format(name, ext)), 'r')
        self.name = name
        self.node_list = set()
        self.time_list = set()
