...
txt_path, json_path, temporal_path = data_dir / "train.txt", data_dir / "test_mini.json", data_dir / "tkbc_processed_data/temporal_model_new.pickle"
...
hipporag = HippoRAG(
        ...
        llm_model_name="casperhansen/llama-3.3-70b-instruct-awq",   # 与 vLLM model id 完全一致
...
# 4. 读取问题 & 答案
queries, answers = load_qas(json_path)
logging.info("Running RAG‑QA on %d queries via OpenAI", len(queries))
print(hipporag.rag_qa(queries=queries,gold_docs=None,gold_answers=answers))
